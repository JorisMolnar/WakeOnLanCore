...
namespace System.Net
{
    ...
    public static class SendWol // TODO: Rename to MagicPacket
    {
        ...
        private static void SendPacket(IPEndPoint target, byte[] packet)
        {
            ...
            {
                cl.Client.Connect(target);
                cl.Client.Send(packet, packet.Length, SocketFlags.None);
            }
        }
        ...
        private static Task SendPacketAsync(IPEndPoint target, byte[] packet)
        {
            ...
            return cl.SendAsync(packet, packet.Length, target).ContinueWith((Task t) => cl.Dispose());
        }
